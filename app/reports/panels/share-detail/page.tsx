...
export default function PanelShareDetailPage() {
  const supabase = getSupabaseClient();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<PanelShareRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase
      ...
      .select('id, created_at, corporate_client_id, lab_order_id, service_name, share_type, share_amount, calculation_basis, settled, corporate_client:corporate_clients(name)')
    ...
    const rows: PanelShareRow[] = ((settlements as any[]) || []).map((s) => {
      const o = s.lab_order_id ? orderMap[s.lab_order_id] : null;
      return {
        id: s.id,
        date: s.created_at,
        panel_name: s.corporate_client?.name || 'Unknown',
  ...
  const columns: ReportColumn<PanelShareRow>[] = [
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold">Panel Share Detail Report</h1>
        <p className="text-sm text-muted-foreground">Individual share transactions for each panel/corporate client</p>
      </div>
      ...
      {loading ? (
        ...
        <ReportTable columns={columns} data={filtered} search={search} searchPlaceholder="Search by panel, patient, invoice..." onSearchChange={setSearch} exportFilename="panel-share-detail" title="Panel Share Detail Report" subtitle={`${filters.dateFrom || 'Today'} to ${filters.dateTo || 'Today'}`} totalsRow={totals} pageSize={50} />
      )}
    </div>
  );
}
