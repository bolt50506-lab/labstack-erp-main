...
export default function DoctorShareSummaryPage() {
  const supabase = getSupabaseClient();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<DoctorDetailRow[]>([]);
  ...
  const filterFields: FilterField[] = [
    ...
    { key: 'doctorId', label: 'Doctor', type: 'select', options: [] },
  ...
  const load = useCallback(async () => {
    ...
    let query = supabase
      ...
      .select(`
        id, doctor_id, lab_order_id, service_name, share_amount, share_percentage,
        gross_amount, discount_amount, net_amount, settled, created_at,
        doctor:doctors(full_name, doctor_code),
    ...
    const rows: DoctorDetailRow[] = ((settlements as any[]) || []).map((s) => {
  ...
  const totals = {
    ...
    share_amount: filtered.reduce((s, r) => s + r.share_amount, 0),
  };

  const columns: ReportColumn<DoctorDetailRow>[] = [
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold">Doctor Share Summary Report</h1>
        <p className="text-sm text-muted-foreground">Detailed per-transaction breakdown of doctor shares with patient, service, and settlement status</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        <Card><CardContent className="pt-4 pb-4"><p className="text-xs text-muted-foreground">Total Share</p><p className="text-sm font-bold text-primary">Rs {formatCurrency(totals.share_amount)}</p></CardContent></Card>
      </div>
      ...
      {loading ? (
        ...
        <ReportTable
          ...
          data={filtered}
          search={search}
          searchPlaceholder="Search doctor, patient, MR no, order..."
          onSearchChange={setSearch}
          exportFilename="doctor-share-summary"
          title="Doctor Share Summary Report"
          subtitle={`${filters.dateFrom || 'Today'} to ${filters.dateTo || 'Today'}`}
          totalsRow={totals as any}
          pageSize={25}
          groupBy={{ label: 'Doctor', getGroupKey: (r) => r.doctor_name }}
        />
      )}
    </div>
  );
